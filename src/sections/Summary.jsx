...
export default function GithubRepoSection({ choice }) {
    ...
    // --- Typed.js ---
    const typedExperienceRef = useRef(null);
    const typedEducationRef = useRef(null);
    useEffect(() => {
        let typedExperienceInstance;
        let typedEducationInstance;

        // Only run Typed.js when the component is in view
        if (isInView && typedExperienceRef.current && typedEducationRef.current) {
            // Ensure text content is cleared before initializing
            typedExperienceRef.current.textContent = '';
            typedEducationRef.current.textContent = '';

            typedExperienceInstance = new Typed(typedExperienceRef.current, {
                strings: ["Experience and"], // Changed first word
                typeSpeed: 80,
                startDelay: 400,
                showCursor: true,
                ...
                onComplete: (self) => {
                    ...
                    typedEducationInstance = new Typed(typedEducationRef.current, {
                        strings: ["Education"],
                        typeSpeed: 80,
                        startDelay: 100,
                        showCursor: true,
        ...
        return () => {
            if (typedExperienceInstance) typedExperienceInstance.destroy();
            if (typedEducationInstance) typedEducationInstance.destroy();
        };
        ...
        >
            ...
            <div className=""> {/* Align items start */}
                ...
                <motion.div
                    ...
                    className={`container mx-auto relative z-10 w-full ${ghBg} border ${ghBorder} rounded-lg shadow-sm ${choice === "development" ? "hover:shadow-cyan-400/20"  : "hover:shadow-yellow-400/20"} overflow-hidden`} // Main repo card styling
                >
