...
export default function Hero({choice}) {
    ...
    useEffect(() => {
        ...
        <div
            ...
            <div
                ref={leftSectionRef}
                className="relative z-10  h-full max-h-[80%] w-full text-left   overflow-auto scrollbar-hide scroll-smooth"
            >
                <div className="h-full md:h-full  flex flex-col justify-center items-start ">
                    <div className={`text-sm md:text-lg mb-5 opacity-50`}><pre className={``}>{choice === 'development' ? (
                        `public static void main(String[] args) {\n\tSystem.out.printf("%s\\n", GREETINGS);\n\tSystem.out.printf("%s", NAME);\n}`
                    ) : (
                        `if __name__ == "__main__":\n\tprint(f"HI THERE, IT'S\\n{NAME}")`
                    )}</pre></div>
